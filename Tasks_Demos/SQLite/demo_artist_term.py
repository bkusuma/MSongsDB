...
def die_with_usage():
    ...
    print('This code gives examples on how to query the database artist_term.db.')
...
if __name__ == '__main__':
    ...
    res = c.execute(q)
    print("* Tables contained in that SQLite file/database (there should be 5: artists, artist_mbtag, artist_term, mbtags, terms):")
    print(res.fetchall())
    ...
    res = c.execute(q1)
    res1_tuple = res.fetchone()
    res1_str = str(res1_tuple[0]) if res1_tuple else "None"
    res = c.execute(q2)
    res2_tuple = res.fetchone()
    res2_str = str(res2_tuple[0]) if res2_tuple else "None"
    ...
    res = c.execute(q)
    print('* MBtags work the same as terms, e.g. list all known mbtags (display first 3):')
    print(res.fetchall()[:3])
    ...
    res = c.execute(q)
    print("* Same thing with musicbrainz tag 'rock':")
    print(res.fetchone())
