...
def die_with_usage():
    ...
    print('This code gives examples on how to query the database track_metadata.db.')
...
if __name__ == '__main__':
    ...
    res = c.execute(q)
    print('* One of the indices we added to the table to make things faster:')
    print(res.fetchone())
    ...
    print('* One unique artist with some track (chosen somewhat arbitrarily by GROUP BY) associated with it:')
    ...
    res = c.execute(q)
    print("* Two track IDs from 'The Beatles', found by looking up the artist by name:")
    ...
    res = c.execute(q)
    print('* One artist having familiarity > 0.8:')
    print(res.fetchone())
    ...
    res = c.execute(q)
    print('* Get the artist with the highest familiarity that has no computed hotttnesss (NB: hotttnesss < 0 means not computed):')
    print(res.fetchone())
